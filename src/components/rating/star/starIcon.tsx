...
export interface StarIconProps extends UiElementProps {
    /**
     * Icon size, 24px is the default size
     */
    size?: string;
    /**
     * State of the star, ["Empty", "Full", "Half"]
     */
    state?: StarIconState;
}
...
export const StarIcon = ({ size, state, className }: StarIconProps) => {
    ...
    const Empty = ({ className }: UiElementProps) => (
        <svg className={`${className}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" width={size ?? SIZE}>
            <path
    ...
    const Half = ({ className }: UiElementProps) =>
    (<svg className={`${className}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" width={size ?? SIZE}>
    ...
    const Full = ({ className }: UiElementProps) => (
        <svg className={`${className}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" width={size ?? SIZE}>
    ...
    const classNameForColor = className ?? `fill-yellow-500`;
    switch (state) {
        case StarIconState.Full:
            return <Full className={classNameForColor} />;
        case StarIconState.Half:
            return <Half className={classNameForColor} />;
        case StarIconState.Empty:
        default:
            return <Empty className={className ?? `fill-blueGray-800`} />;
    }
}