...
export const AnimatedCountdown = ({ boxClassName = '', className = '', ...props }: AnimatedCountdownProps) => {

    const CountdownBox = useCallback(({ title, value }: { title: string, value: number }) => {
        return <div className={`flex flex-col justify-evenly items-center min-w-0
    ...
    return <TimeCountdown {...props} renderer={(interval) => {
        return <article className={`flex gap-2 ${className}`}>
